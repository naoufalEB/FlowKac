...
def feynman_kac(sde, xhat_samples, x, t, args, jacobian_xhat = None, hessian_xhat = None):
    """
    ...
    with torch.no_grad():        
        if args.sto_taylor_order in ["first_order", "first_order_exact"]:
            ...
            xhat_starting_at_x = xhat_samples + torch.matmul(jacobian_xhat[None,...],x[:,None,None,:,None]).squeeze(-1)
